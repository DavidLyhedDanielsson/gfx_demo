...
static double lastFrameTimeMS = 0.0f;
...
namespace DEMO_NAME
{
    void init(HWND hWnd, uint32_t windowWidth, uint32_t windowHeight)
    {
...

        {
            ...
            Die(state.commandQueue->GetTimestampFrequency(&state.timestampFrequency));
        }
...

        {
            ...
            std::memcpy(
                ...
                &transformMatrix,
                state.constants.CBV_TRANSFORM_SIZE);
            // Note the transpose!
...

        {
            device->CreateQueryHeap(
                as_lvalue(D3D12_QUERY_HEAP_DESC{
                    .Type = D3D12_QUERY_HEAP_TYPE_TIMESTAMP,
                    .Count = 2,
                    .NodeMask = 0,
                }),
                Out(state.timestampHeap));
    ...
    void render(uint32_t windowWidth, uint32_t windowHeight)
    {
        ...
        state.commandList->SetGraphicsRootConstantBufferView(
            0,
            state.resources.uploadBuffer->GetGPUVirtualAddress() + state.constants.CBV_TRANSFORM_OFFSET);
        ...
        state.commandList->EndQuery(state.timestampHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, 0);
        state.commandList->DrawIndexedInstanced(state.indexData.size(), 1, 0, 0, 0);
        state.commandList->EndQuery(state.timestampHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, 1);
        state.commandList->ResolveQueryData(
            state.timestampHeap.Get(),
        ...
        uint64_t timingData[2]{};
        void* data;
        state.readbackBuffer->Map(0, nullptr, &data);
        std::memcpy(timingData, data, sizeof(uint64_t) * 2);
        state.readbackBuffer->Unmap(0, nullptr);

        double timeTicks = timingData[1] - timingData[0];
        double timeMS = (timeTicks / state.timestampFrequency) * 1000.0;
        lastFrameTimeMS = timeMS;
    }

    double getLastFrameTimeMS()
    {
