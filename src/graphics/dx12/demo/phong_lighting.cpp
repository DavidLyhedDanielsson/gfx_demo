...
namespace DEMO_NAME
{
    void init(HWND hWnd, uint32_t windowWidth, uint32_t windowHeight)
    {
...

        {
            ...
            for(auto [position, texCoords, normal] =
                    std::make_tuple(mesh->mVertices, mesh->mTextureCoords[0], mesh->mNormals);
...

        {
            ...
            uint32_t i = 0;
            for(const auto [position, uv, normal] : state.vertexData)
            {
                std::memcpy(
        ...
        {
            std::vector vertexShaderCode = FileUtil::readFile(Path::getShaderPath("vs/phong_lighting.bin")).value();
        ...
        {
            std::vector pixelShaderCode = FileUtil::readFile(Path::getShaderPath("ps/phong_lighting.bin")).value();
