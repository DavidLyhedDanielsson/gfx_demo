...
namespace DEMO_NAME
{
    void init(HWND hWnd, uint32_t windowWidth, uint32_t windowHeight)
    {
        ...
        {
            Die(device->CreateHeap(
                as_lvalue(D3D12_HEAP_DESC{
                    .SizeInBytes = state.constants.HEAP_SIZE,
                    .Properties =
                        {
...

        {
            ...
            Die(device->CreateDescriptorHeap(
                ...
                Out(state.descriptorHeaps.srv)));
        }

        auto& descriptorHeapRTV = state.descriptorHeaps.rtv;
        {
...

        {
            device->CreatePlacedResource(
                state.heap.Get(),
                state.constants.HEAP_VERTEX_POSITION_OFFSET,
                as_lvalue(D3D12_RESOURCE_DESC{
                    .Dimension = D3D12_RESOURCE_DIMENSION_BUFFER,
                    .Alignment =
                        0, // https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_desc#alignment
            ...
            device->CreatePlacedResource(
                state.heap.Get(),
                state.constants.HEAP_VERTEX_UV_OFFSET,
                as_lvalue(D3D12_RESOURCE_DESC{
                    .Dimension = D3D12_RESOURCE_DIMENSION_BUFFER,
                    .Alignment =
                        0, // https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_desc#alignment
            ...
            device->CreatePlacedResource(
                state.heap.Get(),
                state.constants.HEAP_VERTEX_INDEX_OFFSET,
                as_lvalue(D3D12_RESOURCE_DESC{
                    .Dimension = D3D12_RESOURCE_DIMENSION_BUFFER,
                    .Alignment =
                        0, // https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_desc#alignment
            ...
            device->CreatePlacedResource(
                state.heap.Get(),
                state.constants.HEAP_TEXTURE_OFFSET,
                as_lvalue(D3D12_RESOURCE_DESC{
...

        {
            device->CreateShaderResourceView(
                ...
                nullptr,
                state.descriptorHeaps.srv->GetCPUDescriptorHandleForHeapStart());
        }
    ...
    void render(uint32_t windowWidth, uint32_t windowHeight)
    {
        ...
        auto backBufferHandle = state.descriptorHeaps.rtv->GetCPUDescriptorHandleForHeapStart();
        ...
        state.commandList->SetDescriptorHeaps(1, state.descriptorHeaps.srv.GetAddressOf());
        state.commandList->SetGraphicsRootDescriptorTable(
            2,
            state.descriptorHeaps.srv->GetGPUDescriptorHandleForHeapStart());
