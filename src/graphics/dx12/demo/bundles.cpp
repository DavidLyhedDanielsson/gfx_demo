...
namespace DEMO_NAME
{
    void init(HWND hWnd, uint32_t windowWidth, uint32_t windowHeight)
    {
        ...
        {
            // There is no need to reset this command allocator
            Die(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_BUNDLE, Out(state.bundleCommandAllocator)));
            Die(device->CreateCommandList(
                0,
                D3D12_COMMAND_LIST_TYPE_BUNDLE,
                state.bundleCommandAllocator.Get(),
                nullptr,
                Out(state.bundleCommandList)));
        }
        ...
        state.bundleCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        state.bundleCommandList->SetGraphicsRootSignature(state.rootSignature.Get());
        state.bundleCommandList->SetPipelineState(state.pipelineState.Get());
        state.bundleCommandList->SetGraphicsRootConstantBufferView(
            0,
            state.resources.uploadBuffer->GetGPUVirtualAddress() + state.constants.CBV_TRANSFORM_0_OFFSET);
        state.bundleCommandList->DrawIndexedInstanced(state.indexData.size(), 1, 0, 0, 0);
        state.bundleCommandList->Close();
    ...
    void render(uint32_t windowWidth, uint32_t windowHeight)
    {
        ...
        state.commandList->ExecuteBundle(state.bundleCommandList.Get());
